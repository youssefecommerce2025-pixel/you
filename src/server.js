...
/* --------------------------------------------------------------------------
 * Prefill TikTok (Login Kit OAuth v2)
 * ------------------------------------------------------------------------ */
const tiktokStates = new Map(); // state -> timestamp
...
function cleanupTiktokStates() {
  const now = Date.now();
  for (const [k, ts] of tiktokStates) {
    if (now - ts > 10 * 60 * 1000) tiktokStates.delete(k);
  }
}
...
app.get("/api/auth/tiktok/start", (req, res) => {
  ...
  if (!clientKey || !clientSecret) {
    return res
      .status(503)
      .send("TikTok non configure. Ajoutez TIKTOK_CLIENT_KEY et TIKTOK_CLIENT_SECRET.");
  }
  cleanupTiktokStates();
...
app.get("/api/auth/tiktok/callback", async (req, res) => {
  ...
  if (error) {
    return res.redirect(
      `/?social_error=${encodeURIComponent(error_description || error)}#devis`
    );
  }
  if (!code || !state || !tiktokStates.has(String(state))) {
    return res.redirect("/?social_error=Session%20TikTok%20invalide#devis");
  }
  tiktokStates.delete(String(state));
  ...
  try {
    ...
    if (!accessToken) {
      return res.redirect("/?social_error=Echec%20token%20TikTok#devis");
    }
    ...
    const user = infoJson.data?.user || infoJson.user || {};
    const display = String(user.display_name || "").trim();
    const parts = display.split(/\s+/).filter(Boolean);
    const prenom = parts[0] || "";
    const nom = parts.slice(1).join(" ") || "";

    // TikTok ne fournit ni email, ni date de naissance, ni code postal.
    ...
    return res.redirect(`/?social_prefill=${payload}#devis`);
  } catch (e) {
    return res.redirect("/?social_error=Erreur%20TikTok#devis");
  }
});
